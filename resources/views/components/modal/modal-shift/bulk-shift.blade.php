    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-2xl shadow-xl">
            ...
            <div class="p-6 text-center">
                ...
                <h3 class="mb-2 text-lg font-bold text-slate-800">Hapus Shift Terpilih?</h3>
                <p class="mb-6 text-sm text-slate-500">Anda akan menghapus <span id="bulk-count" class="font-bold text-slate-900">0</span> shift yang dipilih. Tindakan ini tidak dapat dibatalkan.</p>
                
                <form id="bulk-delete-form" action="{{ route('admin.shifts.bulkDestroy') }}" method="POST" class="flex items-center justify-center gap-3">
                    @csrf
                    @method('DELETE')
                    ...
                    <button type="submit" class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center transition shadow-sm shadow-red-600">
                        Ya, Hapus Semua
                    </button>
                </form>
            </div>
